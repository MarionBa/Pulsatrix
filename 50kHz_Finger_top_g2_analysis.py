...
# Are we studying the first g2 function slope or the second?
FirstSlope = False
SecondSlope = True
...
for file in Files:
    ...
    for g2 in g2_map[int(TimeSample_i*final_fps):int(TimeSample_f*final_fps)]:
        ...
        if FirstSlope == True:
            lag_i = 0  # The lag time we want to consider
            lag_f = 0.0002
        if SecondSlope == True:
            if n == 1:
                # Second slope
        ...
        g2_cropped_g2map.append(g2[int(lag_i * fps):int(lag_f * fps)])

        # g2 slices
        if FirstSlope == True:
            g2_slice1.append(g2[0])
            g2_slice2.append(g2[3])
            g2_slice3.append(g2[6])
        elif SecondSlope == True:
            g2_slice1.append(g2[0])
            g2_slice2.append(g2[int(len(g2)/2)])
            g2_slice3.append(g2[len(g2)-1])
    ...
    #g2map = np.rot90(np.rot90(np.rot90(g2_smoothed[::-1] / np.mean(g2_slice1))))
    # g2map = np.rot90(np.rot90(np.rot90(g2_cropped_g2map[::-1])))
    #
    # # ### Surface plot ###
    # fig, ax = plt.subplots(subplot_kw={"projection": "3d"})
    # x, y = np.meshgrid(time, timeLag)
    #
    # # Plot the surface.
    # surf = ax.plot_surface(x, y, g2map, cmap=cm.coolwarm)
    # ax.set_ylabel(r'$\tau$ [s]')
    # if FirstSlope == True:
    #     ax.set_yticks([0.00002, 0.0001, 0.00015, 0.0002])
    # elif SecondSlope == True:
    #     #ax.set_yticks([0.001, 0.005, 0.01])
    #     print('No ticks')
    ...
    # plt.figure()
    #
    # # Filtered and normalized plots
    # plt.plot(time, filtfilt(b, a, g2_slice1/np.mean(g2_slice1)))
    # plt.plot(time, filtfilt(b, a, g2_slice2/np.mean(g2_slice1)))
    # plt.plot(time, filtfilt(b, a, g2_slice3/np.mean(g2_slice1)))
    # plt.xlabel('Time [s]')
    # plt.xlim([TimeSample_i, TimeSample_f])
    # plt.legend([r'g2(20$\mu$s)/$\bar{g2}$(20$\mu$s)', r'g2(60$\mu$s)/$\bar{g2}$(20$\mu$s)', r'g2(120$\mu$s)/$\bar{g2}$(20$\mu$s)'])


    ### Fitting ###
    FitSecondSlope = 'Exp' #Pol or Log or Exp or Lin
    Inflection_point = 60 * 10 ** -6 # [s]
    Coef = []
    Coef3 = []
    Coef2 = []
    Coef1 = []
    for g2 in g2_map[int(TimeSample_i*final_fps):int(TimeSample_f*final_fps)]:
        if FirstSlope == True:
            ...
            P = np.polyfit(np.arange(1 / fps, Inflection_point, 1 / fps), temp, 1)
            Coef.append(P[0])
        elif SecondSlope == True:
    ...
    if FitSecondSlope == 'Exp' or FitSecondSlope == 'Log':
        ...
        #plt.plot(time, np.array(Coef))
        #plt.plot(time, -Sav_filt)
        # if n == 1:
        #     bpm = 60 / 0.705
        #     bpm_time = np.arange(TimeSample_i + 0.25, TimeSample_f + 0.25, 60 / bpm)
        # elif n == 2:
        #     bpm = 60 / 1.093
        #     bpm_time = np.arange(TimeSample_i + 0.25, TimeSample_f + 0.25, 60 / bpm)
        #
        # for xi in bpm_time:
        #     plt.vlines(xi, min(FiltButter_g2), max(FiltButter_g2), 'k')
        #     print(xi)
